...
**❓ 未定義事項:**
- [x] ~~パスワードポリシー~~ → **基本的なセキュリティにとどめる**。8文字以上 / 英数字混合 / 有効期限なし（変更推奨の通知のみ）
- [x] ~~ログイン試行回数制限~~ → **ロックなし**。ブルートフォース対策はレートリミットで対応（短時間の大量試行をブロック）
...
|------|-------------|
| 現在のパスワード | 必須。一致チェック |
| 新しいパスワード | 8文字以上 / 英数字混合 / 現在のパスワードと異なること |
...
| # | カテゴリ | 未定義事項 | ステータス |
|---|---------|----------|:-----:|
| 1 | 認証 | パスワードポリシー（文字数・複雑性・有効期限） | ✅ M12で解決（8文字以上/英数字混合/有効期限なし） |
...
| 2026-02-13 | v1.2 顧客管理（簡易CRM）画面追加。M16顧客管理（運営向け横断検索/分析）+ S11顧客管理（加盟店向けCRM）。DB3テーブル追加（customers/customer_cards/customer_notes）+ 3ENUM。画面数32→34 |
