...
> AIネイティブ決済代行システム — 全37画面の機能定義
> v1.3: デザインシステム刷新（Stripe風）+ AIチャットグローバル化 + UI修正5件
...
### デザインシステム: Stripe風クリーン 🆕 v1.3

| 要素 | 仕様 |
|------|------|
| ブランドカラー | `#635BFF`（インディゴ/パープル）— brand-50〜brand-900 |
| セマンティックカラー | success（緑）/ warning（黄）/ danger（赤）/ info（青）各50-700 |
| アイコン | Lucide React（94種、tree-shakeable）— 絵文字不使用 |
| フォント | Inter（Latin）+ Noto Sans JP（日本語） |
| サイドバー | ライトテーマ（bg-white + border-r）、アクティブ: brand-50 |
| ボタン | Primary: brand-500、Secondary: white + border、Danger: danger-600 |
| カード | border-slate-200 + rounded-lg + shadow-card |
| モーダル | bg-slate-900/20 + animate-fade-in、パネル: animate-scale-in |
...
| サイドメニュー | Lucideアイコン + ラベル、アクティブ状態: brand-50背景 + brand-700テキスト |
| AIチャット | 全画面共通フローティングパネル（右下Botアイコン → 右サイドw-80展開）。画面別コンテキスト対応サジェスション（17画面分）。ダッシュボードのみデフォルト展開。チャット履歴DB保存 |
| テーブル | ソート（昇順/降順）+ 検索 + フィルタ + ページネーション |
| 確認ダイアログ | 破壊的操作（削除・停止・拒否）は必ずダイアログ表示、animate-scale-in |
| トースト通知 | 操作成功時はsuccess（緑）、エラー時はdanger（赤）、Lucideアイコン付き、3秒で自動消去 |
...
│ │ 書類確認中  │ │ ④ ステップ別の詳細コンテンツ      │
│ ├───────────┤ │                                │
│ │ APP-001   │ │ ⑤ スコープ図（折りたたみ式）🔄v1.3│
│ │ 自社承認済  │ │ [審査フロー全体像 ▶] デフォルト   │
│ └───────────┘ │  非表示。クリックで展開           │
│               │                                │
│               │ ⑥ 操作ボタン                     │
│               │  [承認] [却下] [AI再審査]          │
...
### タブ構成（4タブ）— デフォルト: 審査フロー管理 🔄 v1.3

#### タブ1: 審査フロー管理（7ステップ）— デフォルト表示
...
| 日付 | 内容 |
|------|------|
| 2026-03-03 | v1.3 デザインシステム刷新（Stripe風クリーン）。共通仕様にデザイントークン・AIチャットグローバル追加。M09b接続先審査タブ順序変更（審査フロー管理→審査完了履歴→審査資料管理→接続先一覧）。M04審査フロー全体像を折りたたみ式に変更。M03サイト管理テーブル幅調整 |
