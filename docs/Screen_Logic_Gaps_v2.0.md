...
- 管理者権限での金額編集: 返金モーダル内に「金額変更」リンク（admin以上が表示）
- 金額変更時: `admin_adjusted_amount` に記録 + `admin_adjust_reason` 必須
- **⚠️ 全件上長承認が必要（質問票 C-3 確定）:**
  - admin が金額編集 → ステータス `pending_approval` に設定
  - super_admin が M08 承認キュー で確認 → 承認/却下
  - 承認後に返金/CB金額変更が実行される
  - 承認待ち中は元の取引ステータスは変更しない
...
③ 返金差引額       = SUM(refunds.amount) WHERE status='completed' AND period
④ CB差引額         = SUM(chargebacks.amount) WHERE status IN ('lost','expired') AND period
⑤ リザーブ留保      = (① - ② - ③) × rolling_reserve_settings.reserve_rate / 100
⑥ リザーブ解放      = 手動解放分のみ（運営がM08から解放操作した額）
⑦ 入金額           = ① - ② - ③ - ④ - ⑤ + ⑥
```

> ⚠️ A-5確定: 「決済高に返金分も含まれていればあっている」→ ③返金差引額を明示追加

**留保の計算基準:**
- **手数料・返金差引後 × リザーブ率** — つまり `(① - ② - ③) × rate`
- 理由: 手数料はPSP側の収益、返金分は加盟店に戻るため、実質入金前の額にリザーブをかける

**解放ルール（質問票 A-6 確定: 手動解放のみ）:**
- **自動解放なし** — 運営がM08精算管理画面から手動で解放操作する
- M08に「リザーブ解放」ボタンを実装（対象レコード選択 → 解放額入力 → 実行）
- 解放レコード: `rolling_reserve_transactions` に `type='release'` で記録
- 解放操作は `super_admin` 権限が必要（操作ログ記録）
...
**⚠️ ワンタイムトークン方式（質問票 E-1 確定）:**
- 決済リンクURLにワンタイムトークンを使用
- 1回の決済完了でトークン無効化（`max_uses` のデフォルト = 1）
- 加盟店がリンク作成時に `max_uses` を指定可能（1回〜N回）
- トークンベースのため、URL推測によるアクセス不可
...
**セキュリティ:**
- トークンはワンタイム（使用後に無効化）
- 有効期限: **無期限**（質問票 B-4 確定: ワンタイムトークンのため期限制限不要）
- ~~有効期限: 24時間~~ → `card_change_expires` カラムは NULL許容に変更（無期限 = NULL）
- 未使用トークンの定期クリーンアップ: 日次バッチ（AM 3:00）で `created_at` が90日以上経過 かつ未使用のトークンを削除
- 期限切れ/使用済み時: 「このURLは無効です。新しいカード変更URLを発行してください」エラー表示
...
| 再開 | **Phase 1では不可**（質問票 B-1 方向性: 将来的に再開機能を検討。Phase 2以降で `failed_stopped → active` 遷移を実装予定） |
...
**確定（質問票 D-2）: 15分（PCI DSS v4.0 Req 8.2.8準拠）**

| 項目 | 値 |
|------|-----|
| アイドルタイムアウト | **15分**（全画面共通: マスター管理/加盟店/代理店） |
| PCI DSS準拠 | Req 8.2.8 アイドル15分以内 ✅ |
| 実装 | セッションCookieの `maxAge` + サーバーサイドセッション管理 |
| ユーザー通知 | 残り2分でモーダル「セッションが切れます。延長しますか？」表示 |
| 延長操作 | 「延長」ボタンでセッション延長（15分リセット） |

修正箇所: Screen_Specification — セッションタイムアウト `8時間` → `15分`
